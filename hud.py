"""HUD
Josephine Cattell
This file defines the HUD for the game to the main game
...
class HUD:
    ...
    def _setup_life_image(self):
        self._life_image = pygame.image.load(self.settings.life_image_file)
        self._life_image = pygame.transform.scale(self._life_image, 
            (self.settings.life_image_w, self.settings.life_image_h))
        self.life_rect = self._life_image.get_rect()
    ...
    def _draw_lives(self):
        ...
        for life in range(self.game_stats.ships_left):
            self.screen.blit(self._life_image, (current_x, current_y))
